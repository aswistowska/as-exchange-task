...
export interface Api {
    allSupportedCurrencies(): Promise<Array<Currency>>,
    getQuote(amount: number, base: string, quote: string): Promise<Quote>
}
...
export function getMockApi(): Api {
    return {
        async getQuote(amount: number, base: string, quote: string): Promise<Quote> {
            console.log(amount, base, quote)
            const rate = 2.83;
            return {rate, value: amount*rate};
        },
        async allSupportedCurrencies(): Promise<Array<Currency>> {
